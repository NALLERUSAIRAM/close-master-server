...
const handleTimeout = (room) => {
  ...
  // Cards Show lo timer aithe random ga oka card drop chestham
...
// --- CARDS SHOW (Game 2) PENALTY CALCULATION ---
const calculateCardsShowPoints = (hand) => {
  ...
  let missingValues = missingRanks.map(r => cardValue(r)).sort((a, b) => b - a); // Sort highest first

  // Use Jokers to cancel out the highest missing values
  while (jokerCount > 0 && missingValues.length > 0) {
      missingValues.shift(); 
      jokerCount--;
  }
  return missingValues.reduce((sum, val) => sum + val, 0); // Total Penalty
};
...
io.on("connection", (socket) => {
  ...
  socket.on("create_room", (data, cb) => {
    ...
    const room = {
      roomId,
      gameType: data.gameType || "close_master", // Close Master or Cards Show
  ...
  socket.on("join_room", (data, cb) => {
    ...
    if (data.gameType && room.gameType !== data.gameType) return cb && cb({ error: "Wrong Game Type Room!" });
  ...
  socket.on("start_round", d => {
    ...
    if (r) {
      ...
      const startCards = r.gameType === "cards_show" ? 13 : 7; // 13 Cards for Cards Show
      r.players.forEach(p => {
        p.hand = [];
  ...
  socket.on("action_drop", data => {
    ...
    if (p && p.id === room.turnId && p.hasDrawn) {
      ...
      if (room.gameType === "cards_show") {
        // Game 2: Cards Show Drop Logic (Drop strictly 1 card)
        ...
        // Game 1: Close Master Drop Logic
  ...
  socket.on("exit_room", () => {
      // Room exit logic (same as before)
  });
});
